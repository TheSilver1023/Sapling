...
mc.world.beforeEvents.chatSend.subscribe((ev) => {
	...
	ev.cancel = true;
	if (!commands[name]) return mc.world.sendMessage(`§cInvalid command: '${name.replace('./','')}'`);
	...
	if (vals.includes(null) || vals.length < args.length || vals.length > args.length) return mc.world.sendMessage('§cSyntax error');
